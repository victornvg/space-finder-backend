    ...
    try {
        if(event.queryStringParameters) {
            if(PRIMARY_KEY! in event.queryStringParameters) {
                result.body = await queryWithPartitionKey(event.queryStringParameters)
            } else {
...
async function scanTable() {
    ...
    return JSON.stringify(queryReponse.Items)
}

async function queryWithPartitionKey(queryParams: APIGatewayProxyEventQueryStringParameters) {
